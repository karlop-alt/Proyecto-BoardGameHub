...
def catalogo_view(request):
    """Muestra el catálogo de juegos de mesa disponibles"""
...
def pedidos_view(request):
    """Muestra los pedidos activos para su gestión"""
    pedidos_activos = PedidoDAO.obtener_pedidos_activos()
...
def crear_pedido_action(request):
    """Procesa el formulario web para crear un nuevo pedido"""
    if request.method == 'POST':
        ...
        if cliente_nombre and producto_id:
            ...
            messages.error(
                request,
                "Por favor ingresa tu nombre para procesar el pedido."
            )
...
def cambiar_estado_action(request, pedido_id):
    """Actualiza el estado de un pedido desde la vista de gestión"""
    if request.method == 'POST':
